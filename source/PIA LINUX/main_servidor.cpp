...
int main() {
    ...
    svr.Post("/login", [&](const Request& req, Response& res) {
        try {
            ...
            if (usuarios.count(usuario) && usuarios[usuario] == password) {
                ...
                res.status = 401;
                res.set_content(
                    json{{"error","Credenciales inválidas"}}.dump(4),
                    "application/json"
                );
            }
        } catch (...) {
            res.status = 400;
            res.set_content(
                json{{"error","JSON inválido"}}.dump(4),
                "application/json"
            );
        }
    });
    ...
    svr.Get("/datos", [&](const Request& req, Response& res) {
        if (!req.has_header("Authorization")) {
            res.status = 401;
            res.set_content(
                json{{"error","Falta cabecera Authorization"}}.dump(4),
                "application/json"
            );
            return;
        }
        ...
        if (auth.rfind("Bearer ",0) != 0) {
            res.status = 400;
            res.set_content(
                json{{"error","Formato de token inválido"}}.dump(4),
                "application/json"
            );
            return;
        }
        ...
        if (validarJWT(token, usuario)) {
            ...
            res.status = 401;
            res.set_content(
                json{{"error","Token inválido o expirado"}}.dump(4),
                "application/json"
            );
        }
    });
